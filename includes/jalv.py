...
class Plugin(_ctx):
    ...
    def __init__(self, uri):
        ...
        (
            ...
            self.microVersion,
            self.stability,
        ) = self._get_plugin_version(plugin)
    ...
    def get_jalv_jackname(self):
        ...
        jname = re.sub(
            "[\_]{2,}", "_", re.sub("['\*\(\)\[\]\s]", "_", self.plugin_name)
        )
        jname_count = 0
        while True:
        ...
        for line in output.split("\n"):
            try:
                ...
                if len(parts) == 2:
                    symbol = parts[0]
                    control = next(
                        item for item in self.controls if item["symbol"] == symbol
                    )
    ...
    def _get_port_info(self, port):
        ...
        if port.get_value(world.ns.lv2.shortname):
            errors.append(
                "port '%s' short name is using old style 'shortname' instead of 'shortName'"
                % portname
            )

        # port types
        types = [
            str(t).rsplit("#", 1)[-1][:-4] for t in port.get_value(world.ns.rdf.type)
        ]
        buffer_type = port.get_value(world.ns.atom.bufferType)

        if (
            "Atom" in types
            and port.supports_event(world.ns.midi.MidiEvent)
            and buffer_type
        ...
        if "Control" in types or "CV" in types:
            ...
            if xminimum is not None and xmaximum is not None:
                if is_int:
                    if xminimum.is_int():
                        ...
                        if fmod(ranges["minimum"], 1.0) == 0.0:
                            warnings.append(
                                "port '%s' has integer property but minimum value is float"
                                % portname
                            )
                        else:
                            errors.append(
                                "port '%s' has integer property but minimum value has non-zero decimals"
                                % portname
                            )
                    ...
                    if xmaximum.is_int():
                        ...
                        if fmod(ranges["maximum"], 1.0) == 0.0:
                            warnings.append(
                                "port '%s' has integer property but maximum value is float"
                                % portname
                            )
                        else:
                            errors.append(
                                "port '%s' has integer property but maximum value has non-zero decimals"
                                % portname
                            )
                ...
                if ranges["minimum"] >= ranges["maximum"]:
                    ...
                    errors.append(
                        "port '%s' minimum value is equal or higher than its maximum"
                        % portname
                    )
                    ...
                    if is_int:
                        if xdefault.is_int():
                            ...
                            if fmod(ranges["default"], 1.0) == 0.0:
                                warnings.append(
                                    "port '%s' has integer property but default value is float"
                                    % portname
                                )
                            else:
                                errors.append(
                                    "port '%s' has integer property but default value has non-zero decimals"
                                    % portname
                                )
                ...
                for sp in scalepoints:
                    ...
                    if is_int:
                        if value.is_int():
                            ...
                            if fmod(value, 1.0) == 0.0:
                                warnings.append(
                                    "port '%s' has integer property but scalepoint '%s' value is float"
                                    % (portname, label)
                                )
                            else:
                                errors.append(
                        ...
                        if value.is_int():
                            warnings.append(
                                "port '%s' scalepoint '%s' value is an integer"
                                % (portname, label)
                            )
                            value = int(value) * 1.0
                        else:
                    ...
                    if ranges["minimum"] <= value <= ranges["maximum"]:
                        ...
                        errors.append(
                            ...
                            + ("%d < %d < %d" if is_int else "%f < %f < %f")
                            % (ranges["minimum"], value, ranges["maximum"])
                        )
            ...
            if "enumeration" in properties and len(scalepoints) <= 1:
                errors.append(
                    "port '%s' wants to use enumeration but doesn't have enough values"
                    % portname
                )
    ...
    def _get_plugin_presets(self, plugin):
        ...
        for preset in presets:
            ...
            if labels:
                ...
                label = None
                self.ctx.errors.append("Preset '%s' has no rdfs:label" % preset)
    ...
    def _get_plugin_label(self, plugin):
        ...
        if label is None:
            ...
            self.ctx.warnings.append("plugin label has more than 16 characters")
        return label
    ...
    def _get_plugin_brand(self, plugin):
        ...
        if brand is None:
            ...
            self.ctx.warnings.append("plugin brand has more than 11 characters")
        return brand
    ...
    def _get_plugin_category(self, plugin):
        ...
        if categories:
            for node in categories:
                category.update(
                    self.LV2_CATEGORIES.get(str(node).split("#", 1)[-1], [])
                )
